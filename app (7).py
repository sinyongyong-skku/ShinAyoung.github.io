...
tab1, tab2 = st.tabs(["🌿 식재료 & 레시피", "📸 음식 사진 공유"])
...
with tab1:
    ...
    if not st.session_state.search_done:
        ...
        st.markdown('<p class="sec-sub">카드를 클릭하면 상세 정보를 볼 수 있어요</p>', unsafe_allow_html=True)
...
# ── 레시피 (식재료 탐색 탭 하단에 이어서 표시) ──────────────────────
        ...
        selected_recipe = st.selectbox(
            "레시피 검색",
            options=["— 레시피를 선택해주세요 —"] + recipe_names,
            label_visibility="collapsed",
        )

        if selected_recipe != "— 레시피를 선택해주세요 —":
            for ing in all_ings:
                for rec in ing["recipes"]:
            ...
            st.markdown('<div class="empty">🍽<br><br>위 드롭다운에서 레시피를 선택해주세요</div>', unsafe_allow_html=True)
